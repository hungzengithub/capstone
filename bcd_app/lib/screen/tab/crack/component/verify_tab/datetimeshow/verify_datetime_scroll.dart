...
import 'verify_screen_datetime.dart';

// ignore: must_be_immutable
class VerifyDateTimeScroll extends StatefulWidget {
  final UserDTO dto;
  final List<CrackDTO> listCrack;
  final int statusIndex;
  DateTime selecteVerifydDate;
  VerifyDateTimeScroll(
      this.dto, this.listCrack, this.statusIndex, this.selecteVerifydDate);
  @override
  _DateTimeScroll createState() => _DateTimeScroll();
}

class _DateTimeScroll extends State<VerifyDateTimeScroll> {
  ...
  CrackVerifyListDateTime widgetCrack;
  List<DateTime> markedDates = [];
  List<CrackDTO> listSelectedCrack = new List<CrackDTO>();
  ScrollController _scrollController = ScrollController();
  static DateTime selectedDateSingleton;
  @override
  void initState() {
    super.initState();
    ...
    widgetCrack = CrackVerifyListDateTime(
        widget.dto, listSelectedCrack, widget.statusIndex);
    if (markedDates != null) {
      if (selectedDateSingleton != null) {
        selectedDate = selectedDateSingleton;
      } else if (widget.selecteVerifydDate != null) {
        selectedDate = widget.selecteVerifydDate;
      } else
  ...
  onSelect(data) {
    selectedDateSingleton = data;
    selectedDate = data;
    List<CrackDTO> listSelectedCrack = new List<CrackDTO>();
    widget.selecteVerifydDate = selectedDate;
    for (var crack in widget.listCrack) {
      if (crack.createdDate.difference(selectedDate) < Duration(days: 1) &&
          crack.createdDate.day == selectedDate.day) {
        listSelectedCrack.add(crack);
    ...
    setState(() {
      widgetCrack = new CrackVerifyListDateTime(
          widget.dto, listSelectedCrack, widget.statusIndex);
    });
  }
