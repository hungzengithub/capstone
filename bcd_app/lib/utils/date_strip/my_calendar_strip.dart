...
class MyCalendarStrip extends StatefulWidget {
  ...
  State<MyCalendarStrip> createState() =>
      CalendarStripState(selectedDate, startDate, endDate);
}

class CalendarStripState extends State<MyCalendarStrip>
  ...
  CalendarStripState(
    ...
    lastDayOfMonth = MyDateUtils.getLastDayOfMonth(currentDate);
  ...
  @override
  void didUpdateWidget(MyCalendarStrip oldWidget) {
  ...
  int getLastDayOfMonth(rowStartingDay) {
    return MyDateUtils.getLastDayOfMonth(
  ...
  checkOutOfRangeStatus(DateTime date) {
    ...
    if (!widget.markedDates.contains(date))
      return true;
    else
      return false;
  ...
  build(BuildContext context) {
    return Container(
      height: nullOrDefault(widget.containerHeight, 125.0),
      child: buildDateRow(),
